...
#include "../head/sqlpool.h"

//sql_pool_create(POOL_MAX_NUMBER, "localhost", 3306, "ceshi", "root", "qc123456");
/*创建连接池*/
SQL_CONN_POOL *sql_pool_create(int connect_pool_number)
{
    ...
    {
        log_error("malloc SQL_CONN_POOL error.\n");
        return NULL;
    }
...
int create_db_connect(SQL_CONN_POOL *sp, SQL_NODE *node)
{
    ...
    do
    {
        ...
        /*初始化mysql对象*/
        if ((node->mysql_sock = MySqlInit(&(node->fd)))==NULL)
        {
