...
#define __libSTR(v, buff) (((v) == (unsigned int)-1) ? "*" : (sprintf(buff, "%d", v), buff))
	pos += sprintf(pos, "   tasks_peak  : %s", __libSTR(stat->tasks_peak, buffer1));
	pos += sprintf(pos, "   tasks_added : %s", __libSTR(stat->tasks_added, buffer1));
	pos += sprintf(pos, "tasks_processed: %s", __libSTR(stat->tasks_processed, buffer1));
	pos += sprintf(pos, " tasks_removed : %s", __libSTR(stat->tasks_removed, buffer1));
