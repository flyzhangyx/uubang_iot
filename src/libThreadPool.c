...
static void * admin_thread(void *threadpool)
{
    ...
    while (!pool->shutdown)
    {
        ...
        if (g_queue_size >= MIN_WAIT_TASK_NUM && g_live_thr_num <= pool->max_thr_num)
        {
            ...
            printf("\n[THREAD POOL] %d NEW THREADS CREATE\n",DEFAULT_THREAD_NUM);
        ...
        if ((g_busy_thr_num*2) < g_live_thr_num  &&  g_live_thr_num > pool->min_thr_num)
        {
            ...
            printf("\n[THREAD POOL] RELEASE %d REDUNDANT THREADS \n",DEFAULT_THREAD_NUM);
        }
    }
    return NULL;
}
    ...
    while(1)
    {
        ...
        while ((pool->queue_size == 0) && (!pool->shutdown))
        {
            ...
            if (pool->wait_exit_thr_num > 0)
            {
                ...
                {
                    //printf("\n[THREAD POOL] thread 0x%x is exiting \n", (unsigned int)pthread_self());
                    pool->live_thr_num--;
                    pthread_mutex_unlock(&(pool->lock));
...
int libThreadPool_TaskAdd(threadPool_t *pool, task_func function, void *arg)
{
    ...
    if (pool->shutdown)
    {
        printf("\n[THREAD POOL] THREAD POOL WILL BE SHUTDOWN\n");
        pthread_mutex_unlock(&(pool->lock));
        return -1;
    }
...
threadPool_t * libThreadPool_Init(int min_thr_num, int max_thr_num, int queue_max_size)
{
    ...
    do
    {
        ...
        {
            fprintf(stderr,"\n[THREAD POOL] MALLOC THREADS FALSE\n");
            exit(-1);
        }
        ...
        {
            fprintf(stderr,"\n[THREAD POOL] MALLOC TASK QUEUE FASLE\n");
            exit(-1);
        }
        ...
        {
            printf("\n[THREAD POOL] INIT MUTEX OR COND FALSE\n");
            break;
        }
